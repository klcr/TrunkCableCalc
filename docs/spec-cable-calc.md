...
| 2 | 容量 [kVA] | 選択/数値 | 500 | 標準容量: 50, 75, 100, 150, 200, 300, 500, 750, 1000, 1500, 2000 or カスタム |
| 3 | 一次電圧 [V] | 選択 | 6600 | 3300 / 6600 |
| 4 | 二次電圧 [V] | 選択 | 210 | 105 / 210 / 420 |
| 5 | %Z（パーセントインピーダンス） | 数値 | 自動 | 容量に連動した典型値 or カスタム入力 |
| 6 | 結線方式 | 選択 | Δ-Y | Δ-Y / Y-Y / Δ-Δ / Y-Δ / V-V |

### 19-2. 容量と典型 %Z の対応

JIS C 4304 / JEC 2200 に基づく油入変圧器の代表値:

| 容量 [kVA] | 50 | 75 | 100 | 150 | 200 | 300 | 500 | 750 | 1000 | 1500 | 2000 |
|-----------|-----|-----|------|------|------|------|------|------|------|------|------|
| 典型 %Z | 3.2 | 3.5 | 3.8 | 4.0 | 4.2 | 4.5 | 5.0 | 5.5 | 6.0 | 6.5 | 7.0 |

容量選択時に %Z が「自動」であれば対応する典型値を自動セットする。カスタム入力で上書き可能。

### 19-3. 計算ロジック

```
ソースインピーダンス [Ω]:
  Zs = (%Z / 100) × Vs² / (kVA × 1000)
...
これらは M2-3（一線地絡電流計算: Ig = 3ωCV）で使用する。P4 では null のまま。
...
変圧器二次側の短絡電流を起点に、各ノードのケーブルインピーダンスを加算して算出:
...
ここで:
- Z [Ω/km] は runCalc / runParentCalc で算出した合成インピーダンス（R·cosθ + X·sinθ）
- L [km] はケーブルこう長 [m] / 1000
- Zsource は上流ノードまでの累積インピーダンス
...
- MCCB の遮断容量との参考照合（遮断容量 ≧ Isc₃ であることの確認）
- ツリーテーブルにオプション列として Isc₃ を表示可能

### 20-5. HV 側一線地絡電流（将来実装）
...
P4 では計算は実装せず、§19-5 のデータモデル準備のみ行う。M2-3 で実装予定。
