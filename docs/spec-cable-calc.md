...
**「こう長」の定義:** 供給変圧器の二次側端子（または引込線取付点）から当該ノードまでの電線亘長の合計。feedLink 接続がある場合、上流ノードの亘長も加算する。

#### 8-5a. 亘長ベース電圧降下判定

許容電圧降下率の判定には、ノードの個別ケーブル長ではなく、変圧器からの総亘長を使用する。

**総亘長 ≤ 60m の場合（個別判定）:**
- **幹線ノード:** 変圧器から当該幹線までの幹線区間 VD の合計（幹線累積 VD%）を許容値と比較
- **負荷ノード:** 当該負荷自身の VD% を許容値と比較
- 幹線と負荷はそれぞれ独立に、同じ許容値で判定する

**総亘長 > 60m の場合（累積判定）:**
- 全ノード共通: 変圧器から当該ノードまでの累積 VD% を許容値と比較

#### 8-5b. feedLink 上流VD伝搬

feedLink で接続されたターゲット trunk のサブツリーでは、ソースノードの累積 VD をターゲット側の累積 VD に加算する。これにより、上流の電圧降下が下流の判定に反映される。

### 8-5c. 予備率

負荷条件に「予備率 [%]」を設定可能。デフォルトは 0%（予備なし）。

予備率が設定された場合、設計電流の算出に使用する容量に (1 + 予備率/100) を乗算する:
...
予備率は将来の負荷増設を見込んだケーブルサイズの余裕確保に使用する。ケーブルサイズ選定・MCCB選定・電圧降下計算のすべてに影響する。
...
変圧器配下の全負荷を kVA 換算で集計し、適正容量を評価する。
...
各 load ノードの kVA 換算:
  三相: kVA = dI × √3 × V / 1000
  単相: kVA = dI × V / 1000
  ※ dI は runCalc で算出された需要率適用済み設計電流 [A]

分類集計:
  電動機合計 kVA = Σ(電動機回路の kVA)
  一般合計 kVA = Σ(一般回路の kVA)

総合需要率適用:
  需要 kVA = (電動機合計 + 一般合計) × 総合需要率 / 100
  ※ 総合需要率は変圧器レベルの入力パラメータ（初期値 100%）
```

#### 19-6b. 推奨容量

```
推奨容量 = 需要 kVA 以上の最小標準容量
  （TR_RATINGS の capacities_kVA から走査）
...
三相変圧器に接続される単相負荷の相偏りを評価する。

#### 19-7a. 相割り当てモデル

load ノードに `phase` フィールドを追加:

| 値 | 意味 | 条件 |
|----|------|------|
| `自動` | 三相は均等配分、単相は未割当 | デフォルト |
| `R-S` / `S-T` / `T-R` | 線間接続（200V 系） | 単相2線式 かつ V ≧ 190V |
| `R-N` / `S-N` / `T-N` | 相-中性点接続（100V 系） | 単相2線式 かつ V < 190V |

- 三相負荷（三相3線式 / 三相4線式）は常に3相均等配分
- 単相3線式の trunk 配下は、配下全体を1つの単相負荷として扱う

#### 19-7b. 相別負荷集計

```
phaseLoads = { 'R-S': 0, 'S-T': 0, 'T-R': 0 }

各 load の kVA を相に配分:
  三相: 各相に kVA/3 を加算
  単相（相指定あり — 線間接続）:
    'R-S' → phaseLoads['R-S'] += kVA
    'S-T' → phaseLoads['S-T'] += kVA
    'T-R' → phaseLoads['T-R'] += kVA
  単相（相指定あり — 相電圧接続）:
    'R-N' → phaseLoads['R-S'] += kVA/2, phaseLoads['T-R'] += kVA/2
    'S-N' → phaseLoads['R-S'] += kVA/2, phaseLoads['S-T'] += kVA/2
    'T-N' → phaseLoads['S-T'] += kVA/2, phaseLoads['T-R'] += kVA/2
  単相（'自動'）: 未割当リストに追加（警告表示）
```

#### 19-7c. 不平衡率算出

```
不平衡率 [%] = (最大相 kVA − 最小相 kVA) / 定格容量 × 100
...
変圧器の負荷時電圧降下を Kapp の精密式で算出する。

```
負荷率 k = 需要 kVA / 定格容量 kVA

%R, %X の算出:
  X/R比 が利用可能: %R = %Z / √(1 + (X/R)²), %X = %R × (X/R)
  X/R比 なし: %R = %Z, %X = 0（保守側）

p = %R × k
q = %X × k
cosθ = 加重平均力率（変圧器配下全負荷の P/S 合成）
sinθ = √(1 − cos²θ)

電圧変動率 [%]:
  ε = (p·cosθ + q·sinθ) + (p·sinθ − q·cosθ)² / 200
...
JIS C 4304:2013（油入）/ JIS C 4306:2013（モールド）のトップランナー基準値を参照データとして持ち、ユーザーが手動で上書き可能。

#### 19-9a. 入力

| フィールド | 型 | 初期値 | 説明 |
|-----------|-----|--------|------|
| `wi_W` | number \| null | null（自動） | 無負荷損 [W] |
| `wc_W` | number \| null | null（自動） | 負荷損 [W]（定格時） |
| `lossAuto` | boolean | true | true: JIS 標準値自動取得 / false: 手動入力 |
| `annualLoadFactor` | number \| null | null | 年間負荷率 [%]（入力時のみ年間損失を算出） |

#### 19-9b. 計算ロジック

```
鉄損（無負荷損）Wi [W]: 負荷に関係なく一定
銅損（負荷損）  Pc [W] = Wc × k²（k = 負荷率）
合計損失        Pt [W] = Wi + Pc

効率 [%] = 出力 / (出力 + Pt) × 100
  出力 [W] = 需要 kVA × 1000 × cosθ

最高効率負荷率 [%] = √(Wi / Wc) × 100

年間損失エネルギー [kWh/年]:（年間負荷率入力時のみ）
  = (Wi × 8760 + Wc × (年間負荷率/100)² × 8760) / 1000
```

### 19-10. 励磁突入電流

変圧器投入時の励磁突入電流を推定する。保護協調の参考情報として表示。
...
突入電流倍率:
  油入変圧器: 6〜8 倍（代表値 7 倍）
  モールド変圧器: 8〜12 倍（代表値 10 倍）

ピーク電流 [A] = In × 突入倍率
減衰時間: 油入 約 200 ms、モールド 約 100 ms

※ 実際の突入電流はコア残留磁束、投入位相角、系統インピーダンスに依存する。
  ここでの値は JEC 2200 / IEC 60076-3 の一般的な推定値。
```

### 19-11. LV/LV変圧器（低圧/低圧変換）

#### 19-11a. 概要

低圧幹線間の電圧変換（例: 動力盤 420V → 照明盤 210V、210V → 105V 単相回路）のために、ツリー内の中間ノードとして変圧器を配置できる。既存の transformer ノード型を `category` フィールドで拡張する。

#### 19-11b. ツリー配置ルール

```
HV/LV変圧器（category = 'hv'）: depth 0 のみ（従来通り root）
LV/LV変圧器（category = 'lv'）: depth ≧ 1、親が trunk であること
...
最大深さは 5（LV/LV TR 配下にさらに trunk → load を許容）。

#### 19-11c. 入力項目

| # | 項目 | 型 | 初期値 | 選択肢・範囲 |
|---|------|-----|--------|-------------|
| 1 | カテゴリ | 選択 | hv | hv（高圧/低圧）/ lv（低圧/低圧） |
| 2 | 変圧器名称 | テキスト | LV-TR-1 | 任意 |
| 3 | 変圧器種別 | 選択 | 低圧乾式 | 低圧乾式 / 低圧油入 / 低圧モールド |
| 4 | 容量 [kVA] | 選択/数値 | 10 | 1, 2, 3, 5, 7.5, 10, 15, 20, 30, 50, 75, 100 or カスタム |
| 5 | 一次電圧 [V] | 選択 | 210 | 105 / 200 / 210 / 400 / 420 / 440（親 trunk の電圧を初期値に連動） |
| 6 | 二次電圧 [V] | 選択 | 105 | 一次電圧より低い値のみ選択可能 |
| 7 | %Z | 数値 | 自動 | テーブル連動 or カスタム入力 |
| 8 | X/R 比 | 数値 | 自動 | テーブル連動 or カスタム入力 |
| 9 | 結線方式 | 選択 | Y-Y | Δ-Y / Y-Y / Δ-Δ / Y-Δ |

- category が 'lv' の場合、HV system パラメータセクション（§19-5）は非表示
- 一次電圧は親 trunk の電圧を初期値に自動設定
- 二次電圧は一次電圧より低い値のみ選択可能

#### 19-11d. LV/LV変圧器の容量・%Z・X/R比

低圧乾式変圧器の代表的な値（JIS C 61558-2-4 / メーカーカタログ準拠）:

| 容量 [kVA] | %Z | X/R比 | Wi [W] | Wc [W] |
|---:|---:|---:|---:|---:|
| 1 | 1.50 | 0.20 | 8 | 30 |
| 2 | 1.60 | 0.25 | 14 | 55 |
| 3 | 1.80 | 0.30 | 18 | 75 |
| 5 | 2.00 | 0.40 | 28 | 115 |
| 7.5 | 2.20 | 0.50 | 38 | 165 |
| 10 | 2.40 | 0.60 | 50 | 210 |
| 15 | 2.60 | 0.70 | 68 | 300 |
| 20 | 2.80 | 0.85 | 85 | 385 |
| 30 | 3.00 | 1.00 | 115 | 550 |
| 50 | 3.30 | 1.30 | 175 | 870 |
| 75 | 3.60 | 1.60 | 245 | 1250 |
| 100 | 3.80 | 1.90 | 310 | 1600 |

小容量（≦10kVA）では R 支配的（X/R < 1）、大容量になるに従い X 寄りになる特性がある。

#### 19-11e. 計算ロジック

calcTransformer() は電圧に依存しない汎用ロジックのため、LV/LV でもそのまま適用可能:

```
Rs = (%R/100) × Vs² / S
Xs = (%X/100) × Vs² / S
|Zs| = √(Rs² + Xs²)
In = S / (√3 × Vs)  [三相]  or  S / Vs  [単相]
Isc₃ = Vs / (√3 × |Zs|)
```

ここで Vs は LV/LV TR の二次電圧、S は容量 [VA]。

#### 19-11f. 電圧継承

LV/LV transformer の二次電圧は、その配下の全 trunk ノードの電圧として自動継承される（§19-4 と同じロジック）。

#### 19-11g. 容量検討・不平衡等

§19-6〜§19-10 の解析（容量検討、不平衡率、電圧変動率、損失・効率、突入電流）は LV/LV 変圧器にも同様に適用する。

---

### 19-12. LV/LV変圧器を介した短絡電流の連鎖計算

LV/LV変圧器を通過する際の短絡電流計算:
...
- V1: LV/LV TR の一次電圧 [V]
- V2: LV/LV TR の二次電圧 [V]
- Rcum, Xcum: LV/LV TR の一次側端子における累積インピーダンス [Ω]
- Rs_lv, Xs_lv: LV/LV TR 自身のインピーダンス（§19-11e で算出）

### 19-13. LV/LV変圧器を介した累積電圧降下

```
累積 VD% = 上流 VD% + ε% + 下流 VD%

ε: LV/LV TR の電圧変動率（§19-8 Kapp 精密式で算出）
上流 VD%: LV/LV TR 一次側までの累積電圧降下パーセント
下流 VD%: LV/LV TR 二次側からの電圧降下パーセント（二次電圧基準）
```

### 19-14. LV/LV変圧器の容量集約

親幹線の電流計算では、LV/LV TR 配下の負荷を電力量（kVA）ベースで集約し、親側電圧で電流に変換する:

```
LV TR 需要 [kVA] = 配下負荷合計 [kVA]（需要率適用後）
親幹線側の電流寄与 [A] = LV TR 需要 [kVA] × 1000 / (√3 × V_parent)  [三相]
```
...
LV/LV変圧器が中間に存在する場合:
1. LV/LV TR の一次側端子までの Rcum, Xcum を算出
2. 電圧比変換: R'cum = Rcum × (V2/V1)², X'cum = Xcum × (V2/V1)²
3. LV/LV TR の Rs, Xs を加算
4. LV/LV TR 以降のノードは二次電圧 V2 で Isc₃ を計算
