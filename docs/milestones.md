...
- [ ] M2-3-P1: `ProfileTab` コンポーネント基盤（右ペイン 4 番目タブ、SVG チャート描画エリア）
- [ ] M2-3-P2: 経路抽出ユーティリティ（ツリーから変圧器→末端の全経路を列挙、選択経路のノード配列取得）
- [ ] M2-3-P3: 主曲線描画（Isc₃ 減衰折れ線、ノード点、対数/線形スケール切替）
- [ ] M2-3-P4: しきい値線描画（Iw ケーブル熱耐量、ノード区間ごとの水平ステップ線）
- [ ] M2-3-P5: 経路選択 UI（TreeTable 行クリック連動、◀ ▶ 経路切替、パンくずリスト表示）
- [ ] M2-3-P6: インタラクション（ノード点クリック→左ペイン連動、ホバーツールチップ、交差点強調）
- [ ] M2-3-P7: レイヤートグル（チェックボックスで Icu/Iw/Ig/IΔn 表示切替）
