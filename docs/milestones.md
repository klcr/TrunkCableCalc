...
| M2-3 | 保護設計の可視化 | 短絡電流プロファイルカーブ、遮断容量簡易照合、一線地絡電流・B種接地、ELCB協調 | 未着手 |
...
- [ ] M2-3-B1: `data/mccb-icu.json` 新規作成（フレーム別・電圧別・クラス別 Icu テーブル）
- [ ] M2-3-B2: MCCB 選定 UI に遮断容量クラス選択追加（経済型/高遮断型等）
- [ ] M2-3-B3: 遮断容量チェックロジック（`Icu ≥ Isc₃` 照合、結果に `icuOK` フラグ追加）
- [ ] M2-3-B4: ResultPanel「短絡電流」セクションに遮断容量行 + OK/NG バッジ追加
- [ ] M2-3-B5: プロファイルカーブに Icu しきい値線追加

#### M2-3-G: 一線地絡電流・接地設計

M2-2（HV 系統モデリング）の hvSystem データが前提。

- [ ] M2-3-G1: HV 側地絡電流（`Ig = 3ωCV`）の実装
- [ ] M2-3-G2: B 種接地抵抗計算（`Rb ≤ 150/Ig` or `600/Ig`）
- [ ] M2-3-G3: TransformerResultPanel に「接地設計」セクション追加（Ig / Rb 上限表示）
- [ ] M2-3-G4: プロファイルカーブに Ig しきい値線追加

#### M2-3-E: ELCB 保護協調・LV 側地絡電流

- [ ] M2-3-E1: LV 側地絡電流（対称座標法: `Ig = 3Ea / (2Z₁ + Z₀)`）
- [ ] M2-3-E2: load フォームに ELCB 設定フィールド追加（有無、感度電流 IΔn、接地方式）
- [ ] M2-3-E3: ELCB 保護協調チェック（`RA × IΔn ≤ 50V`）
- [ ] M2-3-E4: ResultPanel に条件付き「地絡保護」セクション追加
- [ ] M2-3-E5: プロファイルカーブに IΔn しきい値線追加
