...
| M2-2 | 高圧系統のモデリング | 受電点から変圧器二次側までの高圧系統をデータモデル化する | 完了 |
| M2-3 | 保護設計の可視化 | 短絡電流プロファイルカーブ、遮断容量簡易照合、一線地絡電流・B種接地、ELCB協調 | 完了 |
...
- [x] M2-2-1: `cable-capacitance.json` に実データ投入（技資第 103 号 A）
- [x] M2-2-2: 変圧器フォームに `hvSystem` セクション追加（HV ケーブル種別/サイズ/こう長/静電容量/VCB遮断時間）
- [x] M2-2-3: HV インピーダンス LV 換算コードの活性化（`calcFaultCurrents` 内スタブ→実コード化）
- [ ] M2-2-4: HV 受電点モデル（`systemMeta.sourceImpedance`、電力会社供給インピーダンス）— 将来拡張
- [x] M2-2-5: VCB/LBS モデリング（VCB 遮断時間フィールド追加、B種接地計算で使用）

### M2-3: 保護設計の可視化

UX 設計思想: `docs/ux-protection-design.md`

#### M2-3-P: 短絡電流プロファイルカーブ（優先）

既存の Isc₃ / Iw データで動作する UI 機能。新しい計算ロジック不要。

- [x] M2-3-P1: `ProfileTab` コンポーネント基盤（右ペイン 4 番目タブ、SVG チャート描画エリア）
- [x] M2-3-P2: 経路抽出ユーティリティ（ツリーから変圧器→末端の全経路を列挙、選択経路のノード配列取得）
- [x] M2-3-P3: 主曲線描画（Isc₃ 減衰折れ線、ノード点、対数/線形スケール切替）
- [x] M2-3-P4: しきい値線描画（Iw ケーブル熱耐量、ノード区間ごとの水平ステップ線）
- [x] M2-3-P5: 経路選択 UI（TreeTable 行クリック連動、◀ ▶ 経路切替、パンくずリスト表示）
- [x] M2-3-P6: インタラクション（ノード点クリック→左ペイン連動、ホバーツールチップ、交差点強調）
- [x] M2-3-P7: レイヤートグル（チェックボックスで Icu/Iw/Ig/IΔn 表示切替）

#### M2-3-B: 遮断容量チェック（簡易照合）

- [x] M2-3-B1: `data/mccb-icu.json` 新規作成（フレーム別・電圧別・クラス別 Icu テーブル）
- [x] M2-3-B2: MCCB 選定 UI に遮断容量クラス選択追加（経済型/高遮断型）
- [x] M2-3-B3: 遮断容量チェックロジック（`Icu ≥ Isc₃` 照合、`calculateAll` step 6 で `icuOK` フラグ追加）
- [x] M2-3-B4: ResultPanel「短絡電流」セクションに遮断容量行 + OK/NG バッジ追加
- [x] M2-3-B5: プロファイルカーブに Icu しきい値線追加（オレンジ破線、レイヤートグル対応）

#### M2-3-G: 一線地絡電流・接地設計

- [x] M2-3-G1: HV 側地絡電流（`Ig = 3ωCV`）の実装（`calcGroundFaultCurrent` 関数）
- [x] M2-3-G2: B 種接地抵抗計算（`Rb ≤ 150/Ig` or `600/Ig`）（`calcGroundingResistance` 関数）
- [x] M2-3-G3: TransformerResultPanel に「接地設計」セクション追加（Ig / Rb 上限表示、VCB 遮断時間別切替）
- [ ] M2-3-G4: プロファイルカーブに Ig しきい値線追加 — 将来拡張（系統レベル値のため水平基準線として設計検討中）

#### M2-3-E: ELCB 保護協調・LV 側地絡電流

- [x] M2-3-E1: LV 側地絡電流（対称座標法: `Ig = 3Ea / (2Z₁ + Z₀)`）（`calcLVGroundFault` 関数、零相比 `Z_ZERO` テーブル使用）
- [x] M2-3-E2: load フォームに ELCB 設定フィールド追加（有無、感度電流 IΔn、接地方式、接地抵抗 RA）
- [x] M2-3-E3: ELCB 保護協調チェック（`RA × IΔn ≤ 50V`）（`checkELCB` 関数、TT/TN 系統対応）
- [x] M2-3-E4: ResultPanel に条件付き「地絡保護」セクション追加
- [x] M2-3-E5: プロファイルカーブに IΔn しきい値線追加（紫破線、レイヤートグル対応）
...
- [x] M2-7-1: 分岐長入力の省略（こう長を「上位接続点〜分岐点」と定義し、分岐ブレーカー判定に兼用。branchLength フィールド除去）
- [x] M2-7-2: 分電盤（分岐盤）の概念導入（幹線ノードに branchName フィールド追加。ブスバー付近にラベル+破線枠表示）
- [x] M2-7-3: JIS C 0617 準拠ブレーカー記号（SvgMCCB を端子線+斜め接点+×マークの JIS シンボルに置換）
